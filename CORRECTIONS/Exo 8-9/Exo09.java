...
public class Exo09 {
	public static void main(String args[]) {
		...
		new Thread(compteJean).start();
		new Thread(compteFlorence).start();
		new Thread(comptePierre).start();
		...
		while (encours) {
			System.out.println(
					"Quelle op�ration souhaitez vous faire ? Depot(D) - Retrait(R) - Cloture(C) - Virement(V) - Quitter(Q) ?");
			...
			try {
				...
				case ("Q"):
					...
					break;
				default:
					System.out.println("Je n'ai pas compris votre dmeande...");
					break;
				}
			} catch (NumberFormatException e) {
				System.out.println("Echec de votre demande, une valeur numérique devait être saisi.");
			}
		}
